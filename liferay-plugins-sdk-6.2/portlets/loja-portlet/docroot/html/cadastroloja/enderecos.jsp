...
if (Validator.isNotNull(lojaId)) {
	loja = LojaUtil.getLoja(Long.valueOf(lojaId));
	endUf = loja.getEndUf();
}
%>

<portlet:resourceURL var="ufResourceURL">
	<portlet:param name="<%=CadastroLojaConstants.METODO %>" value="<%=CadastroLojaConstants.LISTAR_MUNICIPIOS %>"/>
</portlet:resourceURL>
<portlet:resourceURL var="cepResourceURL">
	<portlet:param name="<%=CadastroLojaConstants.METODO %>" value="<%=CadastroLojaConstants.DADOS_CEP %>"/>
</portlet:resourceURL>
...
<div class="row-fluid">
	<aui:fieldset cssClass="span6" label="label.endereco">	
		<aui:field-wrapper helpMessage="help.cep" label="label.endereco.cep" >
			<aui:input name="<%=CadastroLojaConstants.CAMPO_END_CEP %>" label="">
				<aui:validator name="minLength">8</aui:validator>
				<aui:validator name="maxLength">8</aui:validator>
				<aui:validator name="number"></aui:validator>
			</aui:input>
			<c:if test="<%=!CadastroLojaConstants.VIEW.equals(action) %>">
				<liferay-ui:icon
		...
		<aui:input name="<%=CadastroLojaConstants.CAMPO_END_LOGRADOURO %>" label="label.endereco.logradouro" />
		<aui:input name="<%=CadastroLojaConstants.CAMPO_END_NUMERO %>" label="label.endereco.numero" />
		<aui:input name="<%=CadastroLojaConstants.CAMPO_END_COMPLEMENTO %>" label="label.endereco.complemento" />
		<aui:input name="<%=CadastroLojaConstants.CAMPO_END_BAIRRO %>" label="label.endereco.bairro" />
		<aui:select label="label.endereco.uf" name="<%=CadastroLojaConstants.CAMPO_END_UF %>">
			<aui:option label=""
			...
			<aui:option label="MT" selected='<%="MT".equals(endUf)%>'
				value="MT" />
			<aui:option label="MG" selected='<%="MS".equals(endUf)%>'
				value="MS" />
		...
		<aui:input name="<%=CadastroLojaConstants.CAMPO_END_CIDADE %>" label="label.endereco.cidade" />
		<aui:input name="<%=CadastroLojaConstants.CAMPO_END_PAIS %>" label="label.endereco.pais" />
	</aui:fieldset>
</div>

<aui:script use="aui-base">
	var verSeDesabilitaCamposEnderecos = function(desabilita) {
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_CEP %>').set('disabled', desabilita);
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_LOGRADOURO %>').set('disabled', desabilita);
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_NUMERO %>').set('disabled', desabilita);
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_COMPLEMENTO %>').set('disabled', desabilita);
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_BAIRRO %>').set('disabled', desabilita);
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_UF %>').set('disabled', desabilita);
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_CIDADE %>').set('disabled', desabilita);
		A.one('#<portlet:namespace /><%=CadastroLojaConstants.CAMPO_END_PAIS %>').set('disabled', desabilita);
	};

	verSeDesabilitaCamposEnderecos(<%= (CadastroLojaConstants.VIEW.equals(action)) %>);
</aui:script>
