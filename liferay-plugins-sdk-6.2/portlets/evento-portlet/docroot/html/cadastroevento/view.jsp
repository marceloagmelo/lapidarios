...
<%
EventoSearch searchContainer = new EventoSearch(
...
%>
		...
		<aui:nav>
			...
			<c:if test='<%= permissaoConfigEvento %>'>
				<liferay-security:permissionsURL
					modelResource="br.com.prodevelopment.evento"
	...
	<%
		EventoDisplayTerms searchTerms = (EventoDisplayTerms) searchContainer
					.getSearchTerms();

		List<Evento> results = EventoUtil.search(themeDisplay.getCompanyId(), themeDisplay.getScopeGroupId(), 
				searchTerms.getNome(), searchContainer.getOrderByComparator(), themeDisplay.getPermissionChecker());
		...
		for (int i = 0; i < results.size(); i++) {
			Evento evento = (Evento) results.get(i);

			ResultRow row = new ResultRow(evento,
					evento.getEventoId(), i);
			
			PortletURL rowURL = renderResponse.createRenderURL();
			rowURL.setParameter("jspPage", "/html/cadastroevento/cadastro.jsp");
			rowURL.setParameter(Constantes.CAMPO_EVENTO_ID, String.valueOf(evento.getEventoId()));
			rowURL.setParameter(Constantes.ACAO, Constantes.VIEW);
			rowURL.setParameter("backURL", currentURL);
			
			// Nome
			row.addText(HtmlUtil.escape(evento.getNome(locale)),
					rowURL);

			row.addJSP("right", SearchEntry.DEFAULT_VALIGN,
					"/html/cadastroevento/action.jsp",
